...
| 题2 | 容器化持续 CPU Profiling（选做，含 v2.1 增强）| ✅ 已完成 |
...
│   └── visualizations.html  # 题1 可视化汇总（IPC对比/瓶颈矩阵/步长延迟曲线）
└── task2/                   # 题2 容器化 Profiling（选做，已完成 + v2.1 增强）
    ├── README.md            # 题2 说明（架构/启动/示例/8 组件）
    ├── src/                 # Dockerfile + profiler/（8 组件 + 单测）+ supervisord.conf
    ├── test/                # 环境验证 + 3 场景测试 + 火焰图 SVG
    └── ai-chat-log/         # AI 协作对话导出
```

> 题2 Docker 镜像 `profiler.tar.gz`（> GitHub 100MB 限制）放在 **GitHub Release** 附件，下载后 `docker load -i profiler.tar.gz` 即可。**v2.1 含 preflight/metrics/events 增强**，详见 `task2/README.md`。
