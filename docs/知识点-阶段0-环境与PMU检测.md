...
**方法论总纲（贯穿全文，可迁移到任何性能/环境排查）**

1. **判据驱动**：先定合格判据（盯死 4 个关键事件），一次跑完看结果，不瞎试。
   - 4 个事件（cache-misses / L1-dcache-load-misses / LLC-load-misses / dTLB-load-misses）覆盖 cache 层级（L1/LLC）+ TLB，是微架构性能的代表。
   - 一次跑完判生死，避免逐个事件试错浪费时间。
   - 与环境选型的"决策树"（见 `任务分析与环境评估.md` 第四节）配合：判据定标准，决策树走分支。

2. **隔离变量**：把"权限"（paranoid）和"平台能力"（PMU 透传）两个维度拆开归因。
   - 用户态失败时，先提权 root 复测，排除权限因素，再查平台能力。
   - paranoid 管"权限"，`<not supported>` 是"平台能力"——两码事，paranoid 放开救不回平台不支持的事件。

3. **读源头真相**：perf 是用户态工具，最终答案在 `dmesg` / `/sys` / `/proc`，不在 perf 的报错文案里。
   - perf 报错文案常误导（说 `<not supported>` 但不说为什么），内核启动日志和 sysfs 才说真话。
   - 例：WSL2 的 `dmesg` 明说 `unsupported p6 CPU model 151 no PMU driver`——这才是根因，perf 只会说"not supported"。
   - raw code 查询同理：从 `/sys/bus/event_source/devices/armv8_pmuv3_0/events/` 读，不靠 perf 工具映射。
...
**ssh arm 别名**：`~/.ssh/config` 里 `Host arm → HostName <内网 IP>, User root, IdentityFile ~/.ssh/id_arm`，ed25519 密钥免密。
