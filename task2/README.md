...
> **镜像下载**：`profiler.tar.gz`（163MB，aarch64）在 [GitHub Release v2.0](https://github.com/xixirangrangadada/2026CVM-kaohe-wujiahao/releases/tag/v2.0)（因 > GitHub 100MB 限制，放 Release 而非仓库）。下载后与仓库同级执行：
...
## 八、前端界面（Web UI，8080 端口）

启动后浏览器访问 **http://localhost:8080**，单页应用（纯 HTML+JS，无框架）：

- **系统概览**：采集状态（perf 进程在跑）+ 磁盘可用 + 采样文件数（`/api/status`，10s 自动轮询）
- **采样时间线**：列出所有采样文件，点击选中时段（`/api/files`）
- **火焰图查看**：选中时段 → 点"生成火焰图" → iframe 嵌入 SVG，浏览器原生支持交互缩放/搜索（`/api/flame` + `/svg`）

实现：`server.py`（Flask，3 接口 + 静态托管）+ `web/index.html`（复用 C3 query / C4 flamegraph 库，不重复造轮子）。
